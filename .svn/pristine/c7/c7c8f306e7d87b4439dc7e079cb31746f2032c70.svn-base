                  ...
                  self.$confirm('계좌만들기 후 이용 가능합니다.', '', {
                    ...
                    if(self.isDevice)  //모바일앱 
                    {
                        if( (OutBlock1['unit_code']) != '' ){
                           self.$router.push({name: 'mAccCreateResume', params:{ci:window.loginObj['auth_ci'] }});   //계좌만들기 이어서 하기
                        }else
                        {
                           self.$router.push({name: 'mAccCreateIntro', params:{ci:window.loginObj['auth_ci'] }});   //계좌만들기 인트로       
                        }   
                    }
                    else   //모바일웹
                    {
