<template>
	<el-main class="EtcVue SecurePIN">
    <div class="mypage_cont input-form">
      <h3 class="title">{{$t('login.c001')}}<!--보안비밀번호 확인--></h3>
      <el-form ref="form" :model="form" :rules="rule" label-width="0px" class="secure-pin-form">
        <h5>{{$t('login.c002')}}<!--회원님께서 등록한 보안비밀번호를 입력하세요.--></h5>
        <el-form-item prop="pin" :label="$t('signup.a016')">
          <el-input ref="iPass" type="password" autofocus="true" :placeholder="$t('login.c003')" @keypress.native="keypressEvent" auto-complete="new-password" v-model="form.pin" maxlength="4"></el-input>
        </el-form-item>
        <div class="mypage-info login-caution">
          {{$t('login.c004')}}<!-- 보안비밀번호를 잊으셨거나, 인증 시도를 5회 실패한 경우 초기화 후 재등록이 필요합니다. -->
          <el-button type="text" @click="$router.push({name: 'mypageSecurity' ,query:{plan:'preLogin'} })">{{$t('login.c005')}}<!--인증센터로 이동--></el-button>
        </div>
...
export default {
  data () {
    return {
      ...
      form: { pin: '' },
      rule: {
        pin: [{ required: true, message: this.$t('login.c006'), trigger: 'blur' }]
      },
  ...
  mounted() {
    this.$refs.iPass.focus(); //화면 진입시 보안비번 키패드에 focus() 처리
  },
  destroyed() {
