    ...
    <div class="StockSell">
        <!-- <el-scrollbar wrap-class="scrollbar-wrapper" :native="false" class="form-scroll"> -->
            <el-form class="form-area">
                <el-row v-if="isPC" class="able-order">
                    ...
                    <el-col :span="12">
                        <span class="price">{{ getUserId !== null && orderAble != null ? getOrderAbleQtyUnit(orderAble.sell_ord_able_qty) : '' }}</span>
                        {{ trgtCurCd }}
                    </el-col>
                </el-row>
                ...
                <el-form-item class="select-price">
                    ...
                    <el-popover v-if="isPC" placement="bottom-start" trigger="hover">
                        <div class="info">
                            <h2>&lt;{{sellHelpInfo('title', orderType)}}&gt;</h2>
                            <p>{{sellHelpInfo('content', orderType)}}</p>
                        </div>
                ...
                <el-form-item v-if="orderType === '2' || orderType === '3'">
                    <el-col class="inputNumber">
                        <span v-if="isPC" class="order-label">{{$t('exchange.e046')}}<!-- 매도가격 --></span>
                        <el-input ref="price" id="sellPrice" :class="sellPriceChange === true ? 'change-price' : ''" v-model="price" v-bind:disabled="!isPC" @keydown.native="keyDown" @keypress.native="keyPress" @keyup.native="keyUp" @click.native="viewKeyPad('pricePad')">
                ...
                <el-form-item v-if="orderType === '2' || orderType === '3'">
                    <el-col class="inputNumber" :class="{'full':orderType === '2' || (orderType === '3' && !isPC)}">
                        <span v-if="isPC" class="order-label">{{$t('exchange.e045')}}<!-- 매도수량 --></span>
                        <el-input ref="quantity" id="sellQuantity" v-model="quantity" v-bind:disabled="!isPC" @keydown.native="keyDown" @keypress.native="keyPress" @keyup.native="keyUp" @click.native="viewKeyPad('quantityPad')">
                ...
                <el-form-item v-if="orderType === '5'">
                    <el-col class="inputNumber">
                        <span v-if="isPC" class="order-label">{{$t('exchange.e094')}}<!-- 최고가대비하락폭 --></span>
                ...
                <el-form-item v-if="orderType === '1' || orderType === '4' || orderType === '5'">
                    <el-col class="inputNumber" :class="{'full': orderType === '1' || orderType === '4' || orderType === '5'}">
                        <span v-if="isPC" class="order-label">{{$t('exchange.e060')}}<!-- 주문수량 --></span>
                ...
                <el-row class="order-price" v-if="orderType === '2' || orderType === '3'">
                    <el-col :span="7">{{$t('exchange.e024')}}<!-- 주문금액 --></el-col>
                    <el-col :span="17">
        ...
        <div class="order-bottom">
            <el-row class="order-btns">
                ...
                <el-col class="order">
                    <el-button v-on:click="formCheck(true)">{{$t('exchange.e002-1', [trgtCurCd])}}<!-- 매도 --></el-button>
                    <el-dialog :title="$t('exchange.e041')/* 매도주문확인 */" :append-to-body="!isPC ?  true : false " :visible.sync="orderDialogVisible" :class="!isPC ? 'm_body_pop StockSell' : ''" :close-on-click-modal="false" width="360px" center>
                        <el-row class="order-dialog-contents-wrapper">
                            <el-row class="order-dialog-contents">
                                <el-col :span="10" class="order-dialog-contents-title">{{$t('exchange.e019')}}<!-- 종목 --></el-col>
                            ...
                            <el-row class="order-dialog-contents">
                                <el-col :span="10" class="order-dialog-contents-title">{{$t('exchange.e020')}}<!-- 매매구분 --></el-col>
                                <el-col :span="14"><font class="order-dialog-type">{{$t('exchange.e002')}}<!-- 매도 --></font> <span class="unit">({{ orderTypeOption.find(x => x.value == orderType).label }})</span></el-col>
                            </el-row>
                            ...
                            <el-row v-if="orderType === '2' || orderType === '3'" class="order-dialog-contents">
                                <el-col :span="10" class="order-dialog-contents-title">{{$t('exchange.e045')}}<!-- 매도수량 --></el-col>
                                <el-col :span="14">{{ quantity }} <span class="unit">{{ trgtCurCd }}</span></el-col>
                            </el-row>
                            <el-row v-if="orderType === '2' || orderType === '3'" class="order-dialog-contents">
                                <el-col :span="10" class="order-dialog-contents-title">{{$t('exchange.e046')}}<!-- 매도가격 --></el-col>
                                <el-col :span="14">{{ price }} <span class="unit">{{ setlCurCd }}</span></el-col>
                            </el-row>
                            <el-row v-if="orderType === '5'" class="order-dialog-contents">
                                <el-col :span="10" class="order-dialog-contents-title">{{$t('exchange.e094')}}<!-- 최고가대비하락폭 --></el-col>
                            ...
                            <el-row v-if="orderType === '1' || orderType === '4' || orderType === '5'" class="order-dialog-contents">
                                <el-col :span="10" class="order-dialog-contents-title">{{$t('exchange.e060')}}<!-- 주문수량 --></el-col>
                                <el-col :span="14">{{ amount }} <span class="unit">{{ trgtCurCd }}</span></el-col>
                            </el-row>
                            <el-row v-if="orderType === '2'" class="order-dialog-contents">
                                <el-col :span="10" class="order-dialog-contents-title">{{$t('exchange.e024')}}<!-- 주문금액 --></el-col>
                        ...
                        <div slot="footer" class="dialog-footer">
                            ...
                            <el-button type="primary" @click="formCheck(false)">{{$t('exchange.e048')}}<!-- 매도확인 --></el-button>
                        </div>
                    </el-dialog>
                </el-col>
            </el-row>
            ...
            <el-row v-if="!isPC" class="able-order">
                <el-col :span="12">
                    <el-button class="reload" @click="getOrderAble"> {{$t('exchange.e003')}}<!-- 주문가능 --></el-button>
                </el-col>
                <el-col :span="12">
                    <span class="price">{{ getUserId !== null && orderAble != null ? getOrderAbleQtyUnit(orderAble.sell_ord_able_qty) : '' }}</span>
                    {{ trgtCurCd }}
            ...
            <el-dialog :visible.sync="quantityPad" :append-to-body="!isPC ?  true : false " class="keypad amount" :class="!isPC ? 'mobile' : ''" :close-on-click-modal="false" :show-close="false">
                ...
                <el-row class="number-pad">
                    ...
                    <el-col>
                        <el-button @click="setOrdQty('1.00')" v-html="$t('m_keypad.a012')"><!-- 가능수량<br>최대 --></el-button>
                        <el-button @click="setOrdQty('0.50')">50%</el-button>
                        <el-button @click="setOrdQty('0.25')">25%</el-button>
                        <el-button @click="setOrdQty('0.10')">10%</el-button>
                    </el-col>
            ...
            <el-dialog :visible.sync="distancePad" :append-to-body="!isPC ?  true : false " class="keypad amount" :class="!isPC ? 'mobile' : ''" :close-on-click-modal="false" :show-close="false">
                <el-row class="amount-order">
                    <el-col class="small">
                        <div class="label">{{$t('m_keypad.a017')}}<!-- 최고가대비하락폭 --></div>
                        <div class="value_bx">
                            <div class="value_account">
            ...
            <el-dialog :visible.sync="amountPad" :append-to-body="!isPC ?  true : false " class="keypad amount" :class="!isPC ? 'mobile' : ''" :close-on-click-modal="false" :show-close="false">
                ...
                <el-row class="number-pad">
                    ...
                    <el-col>
                        <el-button @click="setAmount('1.00')" v-html="$t('m_keypad.a012')"><!-- 가능수량<br>최대 --></el-button>
                        <el-button @click="setAmount('0.50')">50%</el-button>
                        <el-button @click="setAmount('0.25')">25%</el-button>
                        <el-button @click="setAmount('0.10')">10%</el-button>
                    </el-col>
...
export default {
    data() {
        return {
            ...
            orderData: {
                ...
                symbol : '',                                // O 종목코드
                // tr101
                buy_sell_tp: '1',                           // O 매수매도구분 1.매도 2.매수
                ord_prc_tp: '2',                            // O 주문호가유형 1.시장가, 2.지정가, 3.최유리, 4.최우선, 5.조건부지정가
                ...
                // s0005
                est_dt: '',                                 // O YYYYMMDD                                                       당일날짜 / 당일날짜 / 당일날짜
                sq: '0000000000',                           //                                                                  0000000000 / 0000000000 / 0000000000
                end_dt: '',                                 // O 당일기준 30일이후의 날짜                                         20190111 / 20190111 / 20190111
                at_mnl_ccd: '1',                            // 0:수동 1:자동                                                     1 / 1 / 1
                mbr_ccd: '01',                              // 00:직원 01:고객                                                   01 / 01 / 01
                ip: '',                                     // IPv6 고려됨		                                                 IP / IP / IP
                tmnl_mac_addr: '',                          //                                                                    / /
                md_cd: 'WEB/MTS',                           // WEB, HTS, MTS 정도의 구분임. 클라이언트의 user 필드로 사용          WEB/MTS	/	WEB/MTS	/	WEB/MTS
                lss_rstn_est_f: '',                         // O 0:미설정 1:설정                                                 1 / 1 / 0
                lss_rstn_est_prc_clsf: '',                  // O 1:현재가 2:매도1호가 3:매수1호가                                 1 / 1 /
                lss_rstn_est_unt: '',                       // O 1:가격   2:율  3:Tick                                           1 / 1 /	
                lss_rstn_est_vl: '',                        // O 모든 "값", "수량"의 경우 코인-코인거래를 위해서 (20,8)로 결정됨. 거래가능한 4자리까지는 보여야함. 마스터에서 보여주는 자리수에 대한 정보가 있을것임                스탑가격 / 스탑가격 / 0으로 채움
                prft_rstn_est_f: '0',                       // 0:미설정 1:설정         0 / 0 / 0
                prft_rstn_est_prc_clsf: '',                 // 1:현재가 2:매도1호가 3:매수1호가                                     / /
                prft_rstn_est_unt: '',                      // 1:가격   2:율  3:Tick                                              / /
                prft_rstn_est_vl: '0',                      //                                                                  0으로 채움 / 0으로 채움 / 0으로 채움
                t_s_prft_rstn_est_f: '',                    // O 0:미설정 1:설정                                                 0 / 0 / 1
                t_s_prft_rstn_est_prc_clsf: '',             // O 1:현재가 2:매도1호가 3:매수1호가                                  / / 1
                t_s_prft_rstn_est_unt: '',                  // O 1:가격   2:율  3:Tick                                            / / 1
                t_s_prft_rstn_est_vl: '',                   // O		                                                        0으로 채움 / 0으로 채움 / 주문시점의 현재가
                t_s_hgh_mrk_cmpr_est_unt: '',               // O 1:가격  2:율  3:Tick                                            / / 1
                t_s_hgh_mrk_cmpr_est_vl: '',                // O                                                                0으로 채움 / 0으로 채움 / 최고가대비하락폭
                trd_ccd: '1',                               // 1:매도  2:매수                                                    2 / 2 / 2
                ordr_cnd_dcd: '0',                          // 0:FAS 3:FAK 4:FOK                                                0 / 0 / 0
                ordr_typ_cd: '',                            // O 1:시장가 2:지정가 3:최유리 4:최우선 5:조건부지정가                1 / 2 / 1
                ordr_prc_est_clsf: '',                      // O 1:TICK 2:지정가                                                1 / 2 / 1
                ordr_prc_est_vl: '',                        // O                                                               0으로 채움 / 매도가격 / 0으로 채움
                ordr_q_est_clsf: '2',                       // 1:잔고대비율  2:주 3:금액(신규주문조건설정시)                      2 / 2 / 2
                ordr_q_est_vl: '',                          // O                                                               주문수량 / 매도수량 / 주문수량
                prtn_ordr_f: '0',                           // 0:미설정 1:설정                                                  0 / 0 / 0
                prtn_ordr_tno: '0',                         //                                                                 0 / 0 / 0
                at_crct_est_f: '0',                         // 0:미설정 1:설정                                                  0 / 0 / 0
                at_crct_cmpr_est_vl: '0',                   //                                                                 0 / 0 / 0
                at_crct_est_vl: '0',                        //                                                                 0 / 0 / 0
                at_crct_tno: '0',                           //                                                                 0 / 0 / 0
                proc_sec: '1',                              // 1:신규  2:변경                                                   1 / 1 / 1
            ...
            changedPrice: '0',          // 키패드 매도가격 환산값
            changedDistance: '0',       // 키패드 최고가대비하락폭 환산값
            changedAmount: '0',         // 키패드 주문수량 환산값
            stopPricePad: false,        // 스탑가격 키패드 팝업 여부
            quantityPad: false,         // 매도수량 키패드 팝업 여부
            pricePad: false,            // 매도가격 키패드 팝업 여부
            amountPad: false,           // 주문수량 키패드 팝업 여부
            distancePad: false,         // 최고가대비하락폭 키패드 팝업 여부
            sellPriceChange: false,     // 매도가격 변경 여부
            krwchg: '',                 // 환산값 계산을 위한 결제통화코드 현재가
            trgtKrwchg: '',             // 환산값 계산을 위한 대상통화코드 현재가
            currentBtcKrPrice: 0,       // BTC마켓인 경우 BTC 한화 현재가
            currentEthKrPrice: 0,       // BTC마켓인 경우 ETH 한화 현재가
            name: 'StockSell',
    ...
    computed: {
        ...
        orderSuccessStrings() {
            return [
                '',
                this.$t('exchange.e066'),   // 매도(시장가) 주문이 요청되었습니다.
                this.$t('exchange.e050'),   // 매도(지정가) 주문이 요청되었습니다.
                this.$t('exchange.e079'),   // 매도(스탑 지정가) 주문이 요청되었습니다.<br>현재가격이 스탑가격에 도달하면 주문이 실행됩니다.
                this.$t('exchange.e114'),   // 매도(스탑 시장가) 주문이 요청되었습니다.<br>현재가격이 스탑가격에 도달하면 주문이 실행됩니다.
                this.$t('exchange.e100'),   // 매도(트레일링 스탑) 주문이 요청되었습니다.<br>최고가에서 (최고가대비하락폭)만큼 하락하면 주문이 실행됩니다.
            ]
        },
        guideStrings() {
            return [
                '',
                this.$t('exchange.e106'),   // 시장가격으로 빠르게 매도합니다.
                '',
                this.$t('exchange.e108'),   // 현재가격이 스탑가격에 도달하면 주문이 실행됩니다.
                this.$t('exchange.e108'),   // 현재가격이 스탑가격에 도달하면 주문이 실행됩니다.
                this.$t('exchange.e113'),   // 최고가에서 (최고가대비하락폭)만큼 하락하면 주문이 실행됩니다.
            ]
        },
        orderTypeOption() {
            return [
    ...
    methods: {      // 디자인 적용된 얼럿 메시지
        kvAlert(msg, title, confirm = this.confirmMessage, callback) {
        ...
        keyUp(event) {      // 값 입력 시 자릿수 보정
            ...
            if(targetId === 'stopPrice') {
                ...
                this.amount = this.getOrdQtyUnitValid(this.amount);
            }
        },
        ...
        getOrderAbleQtyUnit(value) {    // 주문가능 수량 소수점 자릿수 처리 - 주문수량 단위 및 단위에 맞춰 0채우기
            if(value === undefined || value === '') {
                return '';
            }
            ...
            return UnitManager.numberWithCommas(NumberUtil.Floor('', '', value, ordQtyBelowPntDigit));
        },
        ...
        setOrdQty(percent) {        // 매도수량 변경(퍼센트,최대)
            var self = this;
            ...
            if(parseFloat(self.getOnlyNumber(self.orderAble.sell_ord_able_qty)) <= 0) {
            ...
            this.$store.dispatch('getOrderAble', payload).then((data) => {
                if(data) {
                    ...
                    // 퍼센트에 대한 계산 값
                    var qty = NumberUtil.multiply(data.sell_ord_able_qty, percent, ordQtyBelowPntDigit, 'floor');

                    if (qty <= 0) {
                        self.quantity = 0;
                    } else {
                        self.quantity = self.addComma(parseFloat(qty).toFixed(ordQtyBelowPntDigit));
                    }
                }
            }, null);
        },
        setAmount(percent) {        // 주문수량 변경(퍼센트,최대)
            var self = this;
            ...
            this.$store.dispatch('getOrderAble', payload).then((data) => {
                if(data) {
                    ...
                    if (qty <= 0) {
                        ...
                        self.amount = self.addComma(parseFloat(qty).toFixed(ordQtyBelowPntDigit));
                    }
                }
            }, null);
        },
        subtractStopPrice() {       // 스탑 매도 가격 빼기
        ...
        setPricePercent(percent) {      // 매도 가격 키패드 - 현재가의 n% 설정
            if(this.curprc <= 0) {
                return;
            }
        ...
        setDistancePercent(percent) {      // 최고가대비하락폭 키패드 - 현재가의 n% 설정
            if(this.curprc <= 0) {
                return;
            }
        ...
        setAveragePrice(type) {     // 스탑가격/가격 키패드 - 매수평균가
            if(type === 'price') {
        ...
        setQuantityKeypad(value) {  // 매도수량 키패드 - 키 입력
            if(value === 'delete') {
        ...
        setPriceKeypad(value) { // 매도가격 키패드 - 키 입력
            if(value === 'delete') {
        ...
        setDistanceKeypad(value) { // 최고가대비하락폭 키패드 - 키 입력
            if(value === 'delete') {
        ...
        setAmountKeypad(value) {    // 주문수량 키패드 - 키 입력
            if(value === 'delete') {
                this.amount = this.amount.toString().substring(0, this.amount.toString().length - 1);
                this.amount = this.getOrdQtyUnitValid(this.amount + value);
            } else if(value === 'close') {
        ...
        getPriceHighLowLimit(type) {        // 매도가격 상하한가 계산
            ...
            limitPrice = this.curprc * limitRatio / 100;  // 주문가격 상한가
        ...
        stopPriceCheck(pre) {       // 스탑가격 유효성 검사
            ...
            if (parseFloat(this.getOnlyNumber(this.stopPrice)) >= parseFloat(this.curprc)) {
                this.kvAlert(this.$t('exchange.e078'));     // 스탑가격을 현재가격보다 낮게 입력하세요.
                this.orderDialogVisible = false;
                return false;
            }
        ...
        distanceCheck(pre) {        // 최고가대비하락폭 유효성 검사
            if(this.distance === undefined || this.distance === '' || this.getOnlyNumber(this.distance) <= 0) {
                this.kvAlert(this.$t('exchange.e097'));     // 최고가대비하락폭을 입력하세요.
                return false;
            }

            if(!this.checkHogaUnit(this.distance)) {
                this.kvAlert(this.$t('exchange.e099'));     // 최고가대비하락폭을 호가단위에 맞게 입력하세요.
                return false;
            }
        ...
        quantityCheck(pre) {        // 매도수량 유효성 검사
            ...
            if(self.orderType === '2' && parseFloat(self.getOnlyNumber(self.quantity)) > parseFloat(self.getOnlyNumber(self.orderAble.sell_ord_able_qty))) {
                self.kvAlert(self.$t('exchange.e031', {'0': self.addComma(self.getOnlyNumber(self.orderAble.sell_ord_able_qty)), '1': self.trgtCurCd}));      // 주문가능 최대수량은 ' + minOrdQty + ' ' + self.trgtCurCd + ' 입니다.
                return false;
            }
        ...
        amountCheck(pre) {      // 주문수량 유효성 검사
            ...
            if(parseFloat(self.getOnlyNumber(self.amount)) < parseFloat(minOrdQty)) {
                self.kvAlert(self.$t('exchange.e032', {'0': minOrdQty, '1': self.trgtCurCd}));      // 주문가능 최소수량은 ' + minOrdQty + ' ' + self.trgtCurCd + ' 입니다.
                return false;
            }
            
            if(self.orderType === '1' && parseFloat(self.getOnlyNumber(self.amount)) > parseFloat(self.getOnlyNumber(self.orderAble.sell_ord_able_qty))) {
                self.kvAlert(self.$t('exchange.e031', {'0': self.addComma(self.orderAble.sell_ord_able_qty.toString()), '1': self.trgtCurCd}));      // 주문가능 최대수량은 ' + minOrdQty + ' ' + self.trgtCurCd + ' 입니다.
                return false;
            }
            ...
            if(self.orderType === '1' && parseFloat(self.getOnlyNumber(self.quantity)) > parseFloat(self.getOnlyNumber(self.orderAble.sell_ord_able_qty))) {
                self.kvAlert(self.$t('exchange.e030'));     // 주문가능 잔고가 부족합니다.
                return false;
            }
        ...
        onSubmit() {    // 서버에 주문 요청
            ...
            if(self.getUserId !== null) {
                ...
                if(self.orderType === '1') {
                    self.orderData.ord_qty = self.getOnlyNumber(self.amount);
                    ...
                    self.orderData.est_dt = today;                              // O YYYYMMDD                                                 당일날짜 / 당일날짜 / 당일날짜
                    self.orderData.end_dt = day30;                              // O 당일기준 30일이후의 날짜                                   20190111 / 20190111 / 20190111

                    self.orderData.lss_rstn_est_f = '1';                        // O 0:미설정 1:설정                                           1 / 1 / 0
                    self.orderData.lss_rstn_est_prc_clsf = '1';                 // O 1:현재가 2:매도1호가 3:매수1호가                           1 / 1 /
                    self.orderData.lss_rstn_est_unt = '1';                      // O 1:가격   2:율  3:Tick                                     1 / 1 /	
                    self.orderData.lss_rstn_est_vl = self.getOnlyNumber(self.stopPrice);            // O 모든 "값", "수량"의 경우 코인-코인거래를 위해서 (20,8)로 결정됨. 거래가능한 4자리까지는 보여야함. 마스터에서 보여주는 자리수에 대한 정보가 있을것임                스탑가격 / 스탑가격 / 0으로 채움

                    self.orderData.t_s_prft_rstn_est_f = '0';                   // O 0:미설정 1:설정                                            0 / 0 / 1
                    self.orderData.t_s_prft_rstn_est_prc_clsf = '';             // O 1:현재가 2:매도1호가 3:매수1호가                            / / 1
                    self.orderData.t_s_prft_rstn_est_unt = '';                  // O 1:가격   2:율  3:Tick                                      / / 1
                    self.orderData.t_s_prft_rstn_est_vl = '0';                  // O		                                                   0으로 채움 / 0으로 채움 / 주문시점의 현재가
                    self.orderData.t_s_hgh_mrk_cmpr_est_unt = '';               // O 1:가격  2:율  3:Tick                                       / / 1
                    self.orderData.t_s_hgh_mrk_cmpr_est_vl = '0';               // O                                                           0으로 채움 / 0으로 채움 / 최고가대비하락폭

                    self.orderData.ordr_typ_cd = '2';                           // O 1:시장가 2:지정가 3:최유리 4:최우선 5:조건부지정가           1 / 2 / 1
                    self.orderData.ordr_prc_est_clsf = '2';                     // O 1:TICK 2:지정가                                           1 / 2 / 1
                    self.orderData.ordr_prc_est_vl = self.getOnlyNumber(self.price);                // O                                                          0으로 채움 / 매도가격 / 0으로 채움
                    self.orderData.ordr_q_est_vl = self.getOnlyNumber(self.quantity);               // O                                                          주문수량 / 매도수량 / 주문수량
                    ...
                    self.orderData.ordr_typ_cd = '1';                           // O 1:시장가 2:지정가 3:최유리 4:최우선 5:조건부지정가           1 / 2 / 1
                    self.orderData.ordr_prc_est_clsf = '1';                     // O 1:TICK 2:지정가                                            1 / 2 / 1
                    self.orderData.ordr_prc_est_vl = '0';                       // O                                                           0으로 채움 / 매도가격 / 0으로 채움
                    self.orderData.ordr_q_est_vl = self.getOnlyNumber(self.amount); // O                                                           주문수량 / 매도수량 / 주문수량
                } else if(self.orderType === '5') {    //트레일링 스탑
                    self.orderData.est_dt = today;                              // O YYYYMMDD                                                  당일날짜 / 당일날짜 / 당일날짜
                    self.orderData.end_dt = day30;                              // O 당일기준 30일이후의 날짜                                   20190111 / 20190111 / 20190111

                    self.orderData.lss_rstn_est_f = '0';                        // O 0:미설정 1:설정                                           1 / 1 / 0
                    self.orderData.lss_rstn_est_prc_clsf = '';                 // O 1:현재가 2:매도1호가 3:매수1호가                            1 / 1 /
                    self.orderData.lss_rstn_est_unt = '';                      // O 1:가격   2:율  3:Tick                                      1 / 1 /	
                    self.orderData.lss_rstn_est_vl = '0';                      // O 모든 "값", "수량"의 경우 코인-코인거래를 위해서 (20,8)로 결정됨. 거래가능한 4자리까지는 보여야함. 마스터에서 보여주는 자리수에 대한 정보가 있을것임                스탑가격 / 스탑가격 / 0으로 채움

                    self.orderData.t_s_prft_rstn_est_f = '1';                   // O 0:미설정 1:설정                                            0 / 0 / 1
                    self.orderData.t_s_prft_rstn_est_prc_clsf = '1';            // O 1:현재가 2:매도1호가 3:매수1호가                            / / 1
                    self.orderData.t_s_prft_rstn_est_unt = '1';                 // O 1:가격   2:율  3:Tick                                      / / 1
                    self.orderData.t_s_prft_rstn_est_vl = self.curprc;          // O		                           0으로 채움 / 0으로 채움 / 주문시점의 현재가
                    self.orderData.t_s_hgh_mrk_cmpr_est_unt = '1';              // O 1:가격  2:율  3:Tick                                       / / 1
                    self.orderData.t_s_hgh_mrk_cmpr_est_vl = self.getOnlyNumber(self.distance);    // O                                                           0으로 채움 / 0으로 채움 / 최고가대비하락폭
                ...
                self.$store.dispatch('coinOrder', self.orderData).then((data) => {
                    ...
                    // 90159    스탑가격을 현재가격보다 낮게 입력하세요.

                    console.log('StockSell order error :', errorData);
        ...
        setTotalAmount() {      // 총 주문금액 계산
            if(!isNaN(this.getOnlyNumber(this.quantity)) && !isNaN(this.getOnlyNumber(this.price)) && (this.getOnlyNumber(this.price) != '0' && this.getOnlyNumber(this.quantity) != '0')) {
                ...
                if(this.setlCurCd !== 'KRW') {
                    var setlDecDigtKRW = this.coinList.length > 0 ? this.$_.findWhere(this.coinList, {o_setl_cur_cd_n : 'KRW'}).o_setl_dec_digt : '';
                    var amtKRW = NumberUtil.multiply(amt, this.krwchg, setlDecDigtKRW, 'round');
                    this.changedPrice = this.addComma(parseFloat(amtKRW));
                }
                this.totalAmount = this.addComma(amt);
                if(!this.isPC && this.setlCurCd !== 'KRW') {
        ...
        setChangedPrice(price) {        // 매도가격 KRW 환산값 계산
            if(this.setlCurCd !== 'KRW') {
                var text = this.getOnlyNumber(price.toString());
                var nLenBelowDigitArray = [1];
        ...
        setChangedDistance(price) {     // 최고가대비하락폭 KRW 환산값 계산
            if(this.setlCurCd !== 'KRW') {
                var text = this.getOnlyNumber(price.toString());
                var nLenBelowDigitArray = [1];
        ...
        setChangedAmount(price) {       // 주문수량 KRW 환산값 계산
            if(this.setlCurCd !== 'KRW') {
                var text = this.getOnlyNumber(price.toString());
                var nLenBelowDigitArray = [1];

                //var amtKRW = text * this.trgtKrwchg;
        ...
        getCurrentSymbolKrwPrice() {        // 대상통화코드 KRW 현재가 가져오기
            var self = this;
            
            // 마스터종목에서 대상, 결제 코드를 가지고 심볼코드를 가져온다
            var symbol = _.findWhere(self.$store.state.data["n1001"], { o_setl_cur_cd_n: 'KRW', o_trgt_cur_cd_n: self.trgtCurCd }).o_symbol;
            
            // var tmpSymbol = self.trgtCurCd;
            // for(var i = 0; i < 5 - self.trgtCurCd.length; i++) {
            //     tmpSymbol += '_';
            // }
            // var symbol = 'KR001' + tmpSymbol + 'KRW__';
            ...
            this.$socket.sendProcessByName('i0018',
                function (queryData) {
                    ...
                    if (queryData != null) {
                        ...
                        self.trgtKrwchg = OutBlock1.curprc;
                    } else {
                        self.trgtKrwchg = 0;
                    ...
                    if(self.amount > 0) {
                        self.setChangedAmount(self.amount);
                    }
                }
            );
        },
        sellInputChange(val) {       // 호가창 매수가격 클릭 시 가격입력 스타일 설정
            if (val !== this.getOnlyNumber(this.price)) {
                this.sellPriceChange = true;
            } else {
                this.sellPriceChange =  false;
            }

            setTimeout(() => this.sellPriceChange = false, 300);
        },
        sellHelpInfo(_part, _orderType) {        // 주문유형 도움말
            var title, content;
            switch(_orderType) {
                case '1':
                    title = this.$t('exchange.e105');       // 시장가(매도)
                    content = this.$t('exchange.e106');     // 시장가격으로 빠르게 매도합니다.
                    break;
                case '2':
                ...
                case '4':
                    ...
                    break;
                case '5':
                    title = this.$t('exchange.e112');       // 트레일링 스탑(매도)
                    content = this.$t('exchange.e113');     // 최고가에서 (최고가대비하락폭)만큼 하락하면 주문이 실행됩니다.
                    break;
            }
