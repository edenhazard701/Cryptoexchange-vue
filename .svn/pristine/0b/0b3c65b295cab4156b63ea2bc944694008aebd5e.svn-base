...
<style type="text/css">
body { margin: 0 auto; font-family:'Malgun Gothic', Dotum,Verdana,AppleGothic,Sans-serif; font-size: 12px; color: #666666; line-height: 1.5; letter-spacing: -0.5px; background-color: #ffffff; }
a { font-weight: bold; color: #116496; text-decoration: none; }
a:hover { color: #1685C8; text-decoration: none; }
...
<%--
//var win = "http://preview.nprotect.net/test/nos_service/win/install/nProtectOnlineSecurity.exe";			// Preview
//var win   = "https://supdate.nprotect.net/nprotect/nos_poc/win/install/nProtectOnlineSecurity.exe";		// POC
var win   = "https://supdated.nprotect.net/nprotect/nos_service/windows/install/nos_setup.exe";		// Real
--%>
	...
	if(npPfsDefine.win) {
		...
		try {
			if(L.db("5.0", npPfsDefine.cR)) {		// XP or Earlier
				//html1.push("<button type=\"button\" onclick=\"doStartInstall('" + winSha1 + "');\">설치파일 다운로드</button>");
				html2.push("<button type=\"button\" onclick=\"location.href='" + winSha1 + "';return false;\">설치파일 다운로드</button>");
				doStartInstall(winSha1);
			} else {
				//html1.push("<button type=\"button\" onclick=\"doStartInstall('" + winSha2 + "');\">설치파일 다운로드</button>");
				html2.push("<button type=\"button\" onclick=\"location.href='" + winSha2 + "';return false;\">설치파일 다운로드</button>");
				doStartInstall(winSha2);
			}
		} catch(e){
			//html1.push("<button type=\"button\" onclick=\"doStartInstall('" + win + "');\">설치파일 다운로드</button>");
		...
		<div class="nos-down">
			...
			<div class="nos-down-msg">
				<ul>
					...
					<li>보안 프로그램 문의안내 : <b>잉카인터넷 고객지원센터 : 1566-0771</b></li>
				</ul>
			</div>
			<div class="nos-line"></div>
		</div>
