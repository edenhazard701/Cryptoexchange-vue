  ...
  <section>
    ...
    <el-dialog title="예약상세" :visible.sync="dialogFormVisible">
      <el-form :model="selectedReservation">
        ...
        <el-form-item label="진료내용" :label-width="formLabelWidth">
          <el-tag v-for="exam in selectedReservation.examinations" :key="exam.id" type="info">{{$store.state.data.getExam(exam.id)[0].name}}</el-tag>
          <!-- <el-radio-group v-model="selectedReservation.preExaminations">
