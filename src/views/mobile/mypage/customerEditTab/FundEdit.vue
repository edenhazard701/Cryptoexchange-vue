        ...
        <el-form ref="fundForm" label-position="left" :model="fundForm" label-width="100px" style="margin:20px 10px 0 20px">
            ...
            <div v-if="fundResouceInputVisible" prop="etcFundResourceType">
                <el-form-item label="기타">      
                    <el-input type="text" placeholder="기타" id="mFundresInputEdit" auto-complete="off"  v-model="fundForm.etcFundResourceType" maxlength="10"></el-input>
                </el-form-item>
            </div>
            ...
            <div v-if="tradePurposeInputVisible" prop="etcTradePurpose">
                <el-form-item label="기타">      
                    <el-input type="text" placeholder="기타" id="mTradePurposeInputEdit" auto-complete="off"  v-model="fundForm.etcTradePurpose" maxlength="10"></el-input>
                </el-form-item>
            </div>
...
export default {   
	...
	methods: { 
        ...
        mFundEdit(data){
            ...
            console.log("mFundEdit_disabled:" +this.editDisabled )   
        },
        ...
        mFundSave(data){               
            ...
            {          
              ...
              //  console.log("saveFundArr:"+  JSON.stringify(this.saveFundArr))
                this.$EventBus.$emit('mFundSendArr', this.saveFundArr);              
            }   
            else{
                this.$EventBus.$emit('mFundSendArr', '');
            }      
        ...
        investInfoUpdate() { 
            ...
            if((self.fundResouceType == 'A99' && $.trim($('#mFundresInputEdit').val()) == '') ||
               (self.tradePurpose == '99' && $.trim($('#mTradePurposeInputEdit').val()) == '')) {
            ...
            self.$EventBus.$emit('mFundSendVali',  self.fundValiRes);
        },
        fundInfoLook() { 
            let self = this;
    ...
    mounted(){
        ...
        if( state == 'saved' ||  
            self.$store.state.user.myInfo.fund_src_tp == '' || self.$store.state.user.myInfo.trd_purp_cd =='')
        {
    ...
    created() {
        this.$EventBus.$on('mFundEdit', this.mFundEdit);    
        this.$EventBus.$on('mFundSave', this.mFundSave);    
        this.$EventBus.$on('mFundVali', this.investInfoUpdate);          
    },   
    beforeDestroy() {
        this.$EventBus.$off('mFundEdit', this.mFundEdit);    
        this.$EventBus.$off('mFundSave', this.mFundSave);          
        this.$EventBus.$off('mFundVali', this.investInfoUpdate);          
