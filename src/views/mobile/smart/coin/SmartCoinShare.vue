<template>
  <div class="SmartCoinShare">
    <div class="deadline">
      <p class="date">10/24</p>
      <p class="txt">
        {{$t('m_mining.c002')}}
        <span class="time">{{miningTime | dividendTimeFormat(this)}}</span>
      </p>
    </div>
    <div class="rate">
      <div class="bullet_question_tit">
        <h2 class="bullet-tit-type01">{{$t('m_mining.c004')}}</h2>
        <el-button class="text-btn question-btn" v-on:click="smartRatioDialogVisible = true">
          <i class="el-icon-question"></i>
        </el-button>
      </div>
      <div class="chart">
        <CoinGraph :data="coinChartData"></CoinGraph>
      </div>
    </div>
    <div class="whole">
      <h2 class="bullet-tit-type01">{{$t('m_mining.c006')}}</h2>
      <el-radio-group v-model="isToday" @change="isTodayChange">
        <el-radio-button label="0">{{$t('m_mining.b012')}}</el-radio-button>
        <el-radio-button label="1">{{$t('m_mining.b013')}}</el-radio-button>
      </el-radio-group>
      <table class="tbody_table_sy">
        <colgroup>
          <col style="width:105px;">
          <col>
        </colgroup>
        <tbody>
          <tr>
            <th scope="row" v-html="$t('m_mining.c007')"></th>
            <td>
              {{currencyFormat(dividendObj['tot_qty'], 0, true)}}
              <span class="s_txt">{{dividend['cur_cd']}}</span>
            </td>
          </tr>
          <tr>
            <th scope="row">
              <span v-html="$t('m_mining.c008')"></span>
              <el-button class="text-btn question-btn" v-on:click="smartAmountDialogVisible = true">
                <i class="el-icon-question"></i>
              </el-button>
            </th>
            <td>
              {{currencyFormat(dividendObj['dvd_amt'], 0, true)}}
              <span class="s_txt">{{amountSymbol}}</span>
            </td>
          </tr>
          <tr>
            <th scope="row" v-html="isToday == '0' ? $t('m_mining.c009') : $t('m_mining.c010')"></th>
            <td>100
              <span class="s_txt">{{dividend['cur_cd']}}</span>
              <span class="ratio_colon">:</span>
              {{currencyFormat(dividendObj['dvd_rt'], 0, true)}}
              <span class="s_txt">{{amountSymbol}}</span>
            </td>
          </tr>
        </tbody>
      </table>
    </div>
    <div class="my_status">
      <div class="bullet_question_tit">
        <h2 class="bullet-tit-type01">{{$t('m_mining.c014')}}</h2>
        <el-button class="text-btn question-btn" v-on:click="smartSituationDialogVisible = true">
          <i class="el-icon-question"></i>
        </el-button>
      </div>
      <div class="status_table">
        <el-table :data="myData">
          <infinite-loading
            :identifier="dividendInfinitedId"
            slot="append"
            @infinite="dividendInfinite"
            force-use-infinite-wrapper=".el-table__body-wrapper"
          ></infinite-loading>
          <el-table-column :label="$t('m_mining.c015')" prop="own" align="center">
            <template slot-scope="scope">
              <span
                :class="chkStrLength(currencyFormat(dividend['avg_poss_amt'], 0, true))"
              >{{currencyFormat(dividend['avg_poss_amt'], 0, true)}}</span>
              <span class="s_txt">{{dividend['cur_cd']}}</span>
            </template>
          </el-table-column>
          <el-table-column :label="$t('m_mining.c016')" prop="rate" align="center">
            <template slot-scope="scope">
              {{currencyFormat(Number(dividend['sta_rt']) * 100, 4)}}
              <span class="s_txt">%</span>
            </template>
          </el-table-column>
          <el-table-column :label="$t('m_mining.c017')" prop="expect" align="center">
            <template slot-scope="scope">
              {{currencyFormat(dividend['pdt_dvd'], 0, true)}}
              <span class="s_txt">{{amountSymbol}}</span>
            </template>
          </el-table-column>
        </el-table>
      </div>
    </div>
    <div class="my_history">
      <h2 class="bullet-tit-type01">{{$t('m_mining.c022')}}</h2>
      <el-table :data="dividendList">
        <el-table-column :label="$t('m_mining.c023')" prop="date" min-width="80" align="center">
          <template slot-scope="scope">
            <p class="date" v-html="dateFormat(scope.row.base_dtm)"></p>
          </template>
        </el-table-column>
        <el-table-column
          :label="$t('m_mining.c024')"
          prop="rate"
          min-width="145"
          align="right"
          header-align="center"
        >
          <template slot-scope="scope">100
            <span class="s_txt">{{dividend['cur_cd']}}</span>
            : {{currencyFormat(scope.row.dvd_rt, 0, true)}}
            <span class="s_txt">{{amountSymbol}}</span>
          </template>
        </el-table-column>
        <el-table-column
          :label="$t('m_mining.c025')"
          prop="mined"
          min-width="95"
          align="right"
          header-align="center"
        >
          <template slot-scope="scope">
            {{priceFormat(scope.row.dvd_pay_cur_cd, scope.row.dvd_pay_cur_cd == "KRW" ? scope.row.dvd_amt : scope.row.depo_amt)}}
            <span class="s_txt_block">{{scope.row.dvd_pay_cur_cd}}</span>
          </template>
        </el-table-column>
      </el-table>
    </div>

    <!-- dialog(배당 비율)// -->
    <el-dialog :visible.sync="smartRatioDialogVisible" :close-on-click-modal="false" center>
      <div class="smart_dialog_txt_wrap">
        <p v-if="$i18n.locale === 'en'">{{$t('m_mining.c005')}} {{dividend['cur_cd']}}</p>
        <p v-else>{{$t('m_mining.c005_01')}} {{dividend['cur_cd']}} {{$t('m_mining.c005_02')}}</p>
      </div>
      <div slot="footer" class="dialog-footer full_footer">
        <el-button type="primary" @click="smartRatioDialogVisible = false">{{$t('m_mining.b008')}}</el-button>
      </div>
    </el-dialog>
    <!-- //dialog(배당 비율) -->
    <!-- dialog(배당액)// -->
    <el-dialog :visible.sync="smartAmountDialogVisible" :close-on-click-modal="false" center>
      <div class="smart_dialog_txt_wrap">
        <p class="top_txt">{{$t('m_mining.c011')}}</p>
        <ul>
          <li class="info">{{$t('m_mining.c012')}}</li>
          <li class="info">{{$t('m_mining.c013')}}</li>
        </ul>
      </div>
      <div slot="footer" class="dialog-footer full_footer">
        <el-button type="primary" @click="smartAmountDialogVisible = false">{{$t('m_mining.b008')}}</el-button>
      </div>
    </el-dialog>
    <!-- //dialog(배당액) -->
    <!-- dialog(나의 배당 현황)// -->
    <el-dialog :visible.sync="smartSituationDialogVisible" :close-on-click-modal="false" center>
      <div class="tit_list_cont">
        <div class="cont">
          <strong class="tit">{{$t('m_mining.c018')}}</strong>
          <ul class="list">
            <li class="dot">{{$t('m_mining.c019')}}</li>
          </ul>
        </div>
        <div class="cont">
          <strong class="tit">{{$t('m_mining.c020')}}</strong>
          <ul class="list">
            <li class="dot">{{$t('m_mining.c021')}}</li>
          </ul>
        </div>
      </div>
      <div slot="footer" class="dialog-footer full_footer">
        <el-button
          type="primary"
          @click="smartSituationDialogVisible = false"
        >{{$t('m_mining.b008')}}</el-button>
      </div>
    </el-dialog>
    <!-- //dialog(나의 배당 현황) -->
  </div>
</template>

<script>
import CoinGraph from "./CoinGraph";
import moment from "moment";
import { mapGetters } from "vuex";
import Vue from "vue";
import InfiniteLoading from "vue-infinite-loading";

/* Infinite-loading
 * guid     : https://peachscript.github.io/vue-infinite-loading/guide/#installation
 * options  : https://peachscript.github.io/vue-infinite-loading/api/#props
 */
Vue.use(InfiniteLoading, {
  slots: {
    noMore: "",
    noResults: ""
  }
});

export default {
  components: {
    CoinGraph,
    InfiniteLoading
  },
  data() {
    return {
      miningTime: "", // 마이닝 종료시까지 남은 시간
      dividendInfinitedId: +new Date(),
      isToday: "1",
      dividendObj: {
        tot_qty: 0,
        dvd_amt: 0,
        dvd_rt: 0
      },
      //팝업
      smartRatioDialogVisible: false,
      smartAmountDialogVisible: false,
      smartSituationDialogVisible: false,
      //전체 마이닝 현황
      amountSymbol: "KRW",
      //나의 마이닝 현황
      myData: [{}],
      coinChartData: {
        labels: ["", ""],
        datasets: [
          {
            label: "배당 비율",
            backgroundColor: [],
            data: [0, 0],
            cur_cd: ""
          }
        ]
      }
    };
  },
  created() {
    let self = this;

    self.coinChartData["labels"] = [self.yesterday(), self.today()];
  },
  methods: {
    /* 숫자 천단위 comma */
    currencyFormat(number, digits, isFloor) {
      if (digits == undefined) digits = 8;

      if (isFloor) {
        if(digits == 0) {
          return Math.floor(number*1).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")
        } else {
          return UnitManager.flooredDecimalWithCommas(number, digits);
        }
      } else {
        return UnitManager.fixedDecimalWithCommas(number, digits);
      }
    },
    priceFormat(cd, prc) {
      if (
        cd == "" ||
        cd == null ||
        cd == undefined ||
        this.hogaUnit == undefined
      )
        return;

      let self = this;
      var nLenBelowDigitArray = [1];
      UnitManager.getHogaUnit(
        self.hogaUnit,
        cd,
        prc,
        undefined,
        undefined,
        nLenBelowDigitArray
      );
      let formatter = Intl.NumberFormat(self.$i18n.locale, {
        minimumFractionDigits: nLenBelowDigitArray[0],
        maximumFractionDigits: nLenBelowDigitArray[0]
      });
      return formatter.format(prc);
    },
    today() {
      const date = new Date();
      const mm = (date.getMonth() + 1).toString();
      const dd = date.getDate().toString();

      return (
        this.$t('m_mining.b013') +"(" + // 당일
        (mm[1] ? mm : "0" + mm[0]) +
        "/" +
        (dd[1] ? dd : "0" + dd[0]) +
        ")"
      );
    },
    yesterday() {
      const date = new Date();
      date.setDate(date.getDate() - 1);
      const mm = (date.getMonth() + 1).toString();
      const dd = date.getDate().toString();

      return (
        this.$t('m_mining.b012') +"(" + // 전일
        (mm[1] ? mm : "0" + mm[0]) +
        "/" +
        (dd[1] ? dd : "0" + dd[0]) +
        ")"
      );
    },
    getDispatchDividend(payload) {
      let self = this;
      if (!payload) payload = {};

      self.$store.dispatch("dividendObj", payload).then(res => {
        //-------------- miningTime 데이터 셋팅 start
        self.miningTime = new Date();
        self.miningTime = new Date(
          self.miningTime.setHours(23 - res.sys_tm.substr(0, 2))
        );
        self.miningTime = new Date(
          self.miningTime.setMinutes(59 - res.sys_tm.substr(2, 2))
        );
        self.miningTime = new Date(
          self.miningTime.setSeconds(59 - res.sys_tm.substr(4, 2))
        );
        //-------------- miningTime 데이터 셋팅 end

        self.coinChartData = {
          labels: [self.yesterday(), self.today()],
          datasets: [
            {
              label: self.$t('mining.c004'), // "배당 비율",
              backgroundColor: ["#e3e3e3", "#1773ee"],
              hoverBackgroundColor: ["#e3e3e3", "#1773ee"],
              data: [
                Number(res["bf_dvd_rt"]).toFixed(2),
                Number(res["to_dvd_rt"]).toFixed(2)
              ],
              cur_cd: "KRW"
            }
          ]
        };
        // 전체 배당 현황  default : 당일
        if (self.isToday == "0") {
          self.dividendObj["tot_qty"] = self.dividend["bf_tot_qty"];
          self.dividendObj["dvd_amt"] = self.dividend["bf_dvd_amt"];
          self.dividendObj["dvd_rt"] = self.dividend["bf_dvd_rt"];
        } else {
          self.dividendObj["tot_qty"] = self.dividend["to_tot_qty"];
          self.dividendObj["dvd_amt"] = self.dividend["to_dvd_amt"];
          self.dividendObj["dvd_rt"] = self.dividend["to_dvd_rt"];
        }
      });
    },
    getDispatchDividendList(payload) {
      let self = this;
      if (!payload) payload = { contiFlag: false };

      /* get data */
      self.$store.dispatch("dividendDetailList", payload).then(res => {
        self.dividendInfinitedId = +new Date();
      });
    },
    dividendInfinite($state) {

      if (!this.$store.state.smart.dividendContiKey) {
        $state.reset()
        return;
      }

      /* add data */
      this.$store
        .dispatch("dividendDetailList", { contiFlag: true })
        .then(res => {
          res.getContiKey() == null ? $state.complete() : $state.loaded();
        });
    },
    isTodayChange(val, old) {
      if (val == "0") {
        // 전일
        this.isToday = "0";
        this.dividendObj["tot_qty"] = this.dividend["bf_tot_qty"];
        this.dividendObj["dvd_amt"] = this.dividend["bf_dvd_amt"];
        this.dividendObj["dvd_rt"] = this.dividend["bf_dvd_rt"];
      } else {
        // 당일
        this.isToday = "1";
        this.dividendObj["tot_qty"] = this.dividend["to_tot_qty"];
        this.dividendObj["dvd_amt"] = this.dividend["to_dvd_amt"];
        this.dividendObj["dvd_rt"] = this.dividend["to_dvd_rt"];
      }
    },
    dateFormat(ord_dtm) {
      if (!ord_dtm) return "";
      ord_dtm = ord_dtm.replace(/\./g, "");
      ord_dtm = ord_dtm.replace(/:/g, "");
      ord_dtm = ord_dtm.replace(/ /g, "");

      return moment(ord_dtm.substr(0, 8) + "T" + ord_dtm.substr(8, 6)).format(
        "YY/MM/DD <br> HH:mm:ss"
      );
    },
    // 글자수 11초과시 다른 class 적용
    chkStrLength(_srt) {
      if (_srt == undefined) return;
      var length = _srt.length;
      if (length > 8) {
        return "fs11";
      }
    }
  },
  computed: {
    ...mapGetters({
      dividend: "getDividend",
      dividendList: "getDividendList",
      hogaUnit: "hogaUnit"
    })
  },
  mounted() {
    let self = this;

    if (self.$store.getters.isSocketConnected) {
      self.getDispatchDividend(null);
      self.getDispatchDividendList(null);

      setInterval(function() {
        self.miningTime = new Date(
          self.miningTime.setSeconds(self.miningTime.getSeconds() - 1)
        );

        // 남은 시간이 매시간 hh시 00분 00초 일 때 마이닝 데이터 재조회
        if (
          self.miningTime.getMinutes() == 59 &&
          self.miningTime.getSeconds() == 58
        ) {
          self.getDispatchDividend(null);
          self.getDispatchDividendList(null);
        }
      }, 1000);
    } else {
      self.$EventBus.$on("socketConnected", () => {
        self.getDispatchDividend(null);
        self.getDispatchDividendList(null);

        setInterval(function() {
          self.miningTime = new Date(
            self.miningTime.setSeconds(self.miningTime.getSeconds() - 1)
          );

          // 남은 시간이 매시간 hh시 00분 00초 일 때 마이닝 데이터 재조회
          if (
            self.miningTime.getMinutes() == 59 &&
            self.miningTime.getSeconds() == 58
          ) {
            self.getDispatchDividend(null);
            self.getDispatchDividendList(null);
          }
        }, 1000);
      });
    }
  },
  filters: {
    dividendTimeFormat(val, self) {
      const tmpFormat = ['분', '초']
      const format = "mm" + tmpFormat[0] + " ss" + tmpFormat[1];
      var res = moment(val).format(format)
          res = res.replace(tmpFormat[0], self.$t("m_mining.b005-2"))
          res = res.replace(tmpFormat[1], self.$t("m_mining.b005-3"))
      return res
    }
  }
};
</script>
