   ...
   <el-main class="bank-account-main">
       ...
       <!-- 본인확인 (휴대폰)-->      
          ...
          <div style="padding-top:50px;" >
            <el-button  style="background-color:#3f50b4;color:white;width:100%;height: 55px;margin-bottom:0px" round @click.native.prevent="checkMobileId" >
              가져오기 ></el-button >
          </div>

           <div style="padding-top:20px;" >
            <el-button  style="background-color:#3f50b4;color:white;width:100%;height: 55px;margin-bottom:0px" round @click.native.prevent="deleteNewRegister" :loading="logining">
              회원가입삭제(테스트) ></el-button >
          </div>
...
export default {
  ...
  methods: {
    ...
    self.myKeepInCert()
      ...
      self.stateData = '08eeb103-2f6e-0e01-3cc5-730b72a50c45'
      self.codeData = '8dd20140-7498-4a9b-9603-351e2a08a169'    
   ...
   //  self.getMyKeepInVerifiedResult(self.stateData,self.codeData )
    ...
    //회원가입 삭제용
    deleteNewRegister(param){
         	let self = this;   
          self.$socket.sendProcessByName('ac103', (queryData) => {
          let block = queryData.getBlockData('InBlock1')[0];
              block['auth_ci'] = 'nxnluP/9PdyIxtsqgi5mHuXiAjc1ziIDCTR5qx+e61wDdvyQPB0X0pQl/cM3CuW6wsv8bt+032b/qSbXn9bFVw=='  //param//       
          ...
          if (queryData != null) {           
            ...
            // 전문 에러 언어팩 적용
            const errorData = JSON.parse( window.sessionStorage.getItem('lastErrorData') );
            if (errorData.trName != "ac103") return
